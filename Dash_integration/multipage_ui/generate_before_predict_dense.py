...
def main(wav_file, flag_for_data, data):
    ...
    #############################################################################
                # If flag_for_data = 1 then return only predictions
    #############################################################################
    elif flag_for_data == 1:
        predict_prob, predictions = model_function_dense.predictions_wavfile(data)
        K.clear_session()
